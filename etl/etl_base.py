...
class BreezewayETL:
    ...
    def _transform_children(self, parent_record: Dict, child_name: str,
        ...
        for child in child_records:
            ...
            # For guests
                ...
                if child.get('name') is None and child.get('email') is None and child.get('phone') is None:
                    continue  # skip content-less Breezeway guest items; prevents all-NULL runaway (incident 2026-06-25)
                transformed_child.update({
    ...
    def _upsert_parents(self, cur, records: List[Dict]) -> Tuple[int, int]:
        ...
        # Build update clause (all columns except conflict keys and any the
        # entity marks no_update_columns). property_status is owned by the
        # real-time Breezeway webhook (supabase_sync / handlers); the poll must
        # set it on INSERT but never clobber it on UPDATE.
        no_update_columns = set(self.entity_config.get('no_update_columns', []))
        update_columns = [
            c for c in columns
            if c not in conflict_keys and c != 'created_at' and c not in no_update_columns
        ]
        update_clause = ', '.join([
            f"{col} = EXCLUDED.{col}"
