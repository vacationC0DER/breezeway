#!/usr/bin/env python3
"""
reservation_tasks_supabase_push.py
==================================
Push breezeway.reservation_tasks (+ a curated jsonb fold of task_requirements)
to the VR Goals Supabase table tasks.bz_reservation_tasks, via a service_role
PostgREST upsert (Content-Profile: tasks). Multitenant: client_id is resolved
server-side by the bz_resolve_reservation_task_client_id trigger — we never send
client_id.

Incremental by source synced_at watermark, tracked in breezeway.supabase_push_log.
The source table already holds only 2026+ reservation tasks, so a --full run is a
complete backfill.

Usage:
    python3 etl/reservation_tasks_supabase_push.py all
    python3 etl/reservation_tasks_supabase_push.py austin --full
    python3 etl/reservation_tasks_supabase_push.py austin --full --limit 200 --dry-run
"""
...
TARGET_TABLE = 'bz_reservation_tasks'
CONFLICT = 'region_code,reservation_id,breezeway_task_id'
BATCH = 500
HTTP_TIMEOUT = 30
...
logger = logging.getLogger('ResTasksPush')

# Source columns -> target columns (rename a few; rest are 1:1).
COLS = [
    'region_code', 'reservation_id', 'task_id', 'reservation_pk', 'task_pk',
    'home_id', 'reference_property_id', 'task_name', 'task_description',
    'type_department', 'type_priority', 'task_status_code', 'task_status_name',
    'task_status_stage', 'scheduled_date', 'scheduled_time', 'task_created_at',
    'started_at', 'finished_at', 'paused', 'rate_type', 'rate_paid',
    'total_time', 'template_id', 'report_url', 'assignments', 'supplies',
    'costs', 'tags', 'task_tags', 'raw_payload', 'synced_at',
]
RENAME = {
    'task_id': 'breezeway_task_id',
    'home_id': 'breezeway_property_id',
    'synced_at': 'source_synced_at',
}
# id-ish columns forced to text to match the target schema.
TEXT_COLS = {'reservation_id', 'breezeway_task_id', 'breezeway_property_id',
             'reference_property_id', 'template_id', 'total_time', 'type_priority'}
...
def fetch_requirements_map(conn, task_pks):
    """task_pk -> [curated requirement objects]. Heavy `photos` arrays excluded."""
    if not task_pks:
        return {}
    with conn.cursor(cursor_factory=RealDictCursor) as cur:
        cur.execute("""
            SELECT task_pk,
                   ...
                   ) ORDER BY section_name, requirement_id) AS reqs
            FROM breezeway.task_requirements
            WHERE task_pk = ANY(%s)
            GROUP BY task_pk
        """, (list(task_pks),))
        return {r['task_pk']: r['reqs'] for r in cur.fetchall()}


def to_payload(src_row, req_map):
    out = {}
    for c in COLS:
    ...
    for k in TEXT_COLS:
        if out.get(k) is not None:
            out[k] = str(out[k])
    out['requirements'] = req_map.get(src_row['task_pk'])
    return out
...
def run_region(conn, region, full, limit, dry):
    ...
    sql = f"""
        SELECT {', '.join(COLS)}
        FROM {SRC_SCHEMA}.reservation_tasks
        WHERE {where}
        ORDER BY synced_at
    """
    if limit:
    ...
    for i in range(0, total, BATCH):
        ...
        req_map = fetch_requirements_map(conn, {r['task_pk'] for r in chunk if r['task_pk']})
        payload = [to_payload(r, req_map) for r in chunk]
...
def main():
    p = argparse.ArgumentParser(description='Push breezeway.reservation_tasks -> Supabase')
