...
SUPABASE_SERVICE_ROLE_KEY = _ENV.get("SUPABASE_SERVICE_ROLE_KEY") or ""
...
def _seed_supabase_region_map(region_code: str, client_id: int, company_id: str) -> None:
    """Best-effort: seed Supabase tasks.bz_region_client_map so the property_status
    and reservation_tasks pushes can resolve client_id for this region without a
    manual step. Non-fatal — the VPS region add already succeeded."""
    if not (SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY):
        log.warning("Supabase creds unset — skipping bz_region_client_map seed for %s", region_code)
        return
    try:
        resp = requests.post(
            f"{SUPABASE_URL}/rest/v1/bz_region_client_map?on_conflict=region_code",
            json={
                "region_code": region_code,
                "client_id": client_id,
                "breezeway_company_id": int(company_id) if company_id.isdigit() else None,
                "is_active": True,
            },
            headers={
        ...
        if resp.status_code >= 300:
            log.error("bz_region_client_map seed failed [%s]: %s", resp.status_code, resp.text[:200])
        else:
            log.info("seeded bz_region_client_map: %s -> client %s", region_code, client_id)
    except Exception as e:  # noqa: BLE001
        log.error("bz_region_client_map seed error for %s: %s", region_code, e)
...
def add_region(tenant_id: int, body: RegionCreate):
    ...
    _seed_supabase_region_map(body.region_code, tenant_id, body.breezeway_company_id)
